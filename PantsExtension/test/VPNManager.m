...
- (NETunnelProviderManager *)createProviderManager{
    ...
    conf.serverAddress = @"Pants VPN";
    manager.protocolConfiguration = conf;
    manager.localizedDescription = @"Pants VPN";
    return manager;
}
...
- (void)loadAndCreatePrividerManager:(void(^)(NETunnelProviderManager *manager))compelte{
    [NETunnelProviderManager loadAllFromPreferencesWithCompletionHandler:^(NSArray<NETunnelProviderManager *> * _Nullable managers, NSError * _Nullable error) {
        ...
        conf[@"ss_address"] = getObjectFromUserDefaults(kSeverAddress);
        conf[@"ss_port"] = getObjectFromUserDefaults(kPort);
        conf[@"ss_method"] = getObjectFromUserDefaults(kMethod);// 大写 没有横杠 看Extension中的枚举类设定 否则引发fatal error
        conf[@"ss_password"] = getObjectFromUserDefaults(kPassword);
